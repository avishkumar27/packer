...
source "proxmox-iso" "ubuntu-server" {
    ...
    ## Replace your network drives and ip config in the boot command below
    # PACKER Boot Commands
    boot_command = [
        "c<wait>linux /casper/vmlinuz autoinstall ip=<ip>::<gw>:<subnet mask>::::<dns> ds='nocloud-net;s=http://{{ .HTTPIP }}:{{ .HTTPPort }}/'<enter><wait5s>initrd /casper/initrd <enter><wait5s>boot <enter><wait5s>"
    ]
    #boot = "c"
    boot_wait = "5s"
